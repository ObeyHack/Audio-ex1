...
def do_istft(spec: torch.Tensor, n_fft: int=1024) -> torch.Tensor:
    ...
    """
    istft = torch.istft(spec, n_fft=n_fft, hop_length=n_fft//4, win_length=n_fft, return_complex=False)
    return istft
...
def plot_spectrogram(wav: torch.Tensor, n_fft: int=1024) -> None:
    ...
    mag = torch.sqrt(stft.pow(2).sum(-1))
    if len(mag.shape) == 3:
        mag = mag.squeeze(0)
    plt.imshow(mag, aspect='auto', origin='lower', cmap='inferno')
    plt.xlabel('Time (s)')
